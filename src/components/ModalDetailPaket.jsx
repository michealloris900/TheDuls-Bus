...
const ModalDetailPaket = ({ paket, onClose }) => {
  ...
  const getFasilitasIcon = (item) => {
    if (item.includes('AC')) return '❄️';
    if (item.includes('TV')) return '📺';
    if (item.includes('Karaoke')) return '🎤';
    if (item.includes('Reclining')) return '💺';
    if (item.includes('WiFi')) return '📶';
    if (item.includes('Audio')) return '🔊';
    if (item.includes('Snack')) return '🍪';
    if (item.includes('Jeep')) return '🚙';
    return '✨';
  };
        ...
        <div className="relative h-72 md:h-96">
          ...
          <div className="absolute bottom-6 left-6 right-6">
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="bg-white/20 backdrop-blur-md text-white px-3 py-1 rounded-full text-sm flex items-center">
            ...
            <h2 className="text-3xl md:text-4xl font-bold text-white">{paket.nama}</h2>
          </div>
        </div>

        {/* Content */}
        <div className="p-6 md:p-8">
          {/* Info Grid - AMBIL DARI DATA */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            <div className="bg-blue-50 rounded-xl p-4 text-center hover:bg-blue-100 transition-colors">
              ...
              <div className="font-bold text-gray-800 text-sm truncate" title={paket.hotel}>
                {paket.hotel.length > 20 ? paket.hotel.substring(0, 18) + '...' : paket.hotel}
              </div>
            </div>
            <div className="bg-green-50 rounded-xl p-4 text-center hover:bg-green-100 transition-colors">
              ...
              <div className="font-bold text-gray-800 text-sm">{paket.armada}</div>
            </div>
            <div className="bg-purple-50 rounded-xl p-4 text-center hover:bg-purple-100 transition-colors">
              ...
              <div className="font-bold text-gray-800">{paket.kapasitasGroup || "Up to 50 org"}</div>
            </div>
            <div className="bg-orange-50 rounded-xl p-4 text-center hover:bg-orange-100 transition-colors">
              <div className="text-2xl mb-1">⭐</div>
              <div className="text-sm text-gray-500">Rating</div>
              <div className="font-bold text-gray-800">
                {paket.rating}/5.0 • ({paket.totalUlasan})
              </div>
            </div>
          </div>
          ...
          <div className="mb-8">
            ...
            <div className="space-y-3">
              {paket.itinerary.map((item, index) => (
                <div 
                  key={index} 
                  className="flex items-start p-3 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors group"
                >
                  <span className="bg-blue-600 text-white w-6 h-6 rounded-full flex items-center justify-center text-sm font-bold mr-3 group-hover:scale-110 transition-transform">
                    {index + 1}
                  </span>
          ...
          <div className="mb-8">
            <h3 className="text-xl font-bold text-gray-800 mb-4 flex items-center">
              <span className="bg-green-600 w-1 h-6 rounded-full mr-3"></span>
              Semua Include
            </h3>
          ...
          {/* Fasilitas Unggulan - AMBIL DARI DATA */}
          ...
          <div className="border-t border-gray-200 pt-6">
            <div className="flex flex-col md:flex-row justify-between items-center gap-4">
              <div>
                <p className="text-gray-500 mb-1">Harga Spesial Grup</p>
                <p className="text-4xl font-bold text-blue-600">
                  {formatHarga(paket.harga)}
                  <span className="text-lg font-normal text-gray-500 ml-2">/group</span>
                </p>
                <p className="text-sm text-gray-500 mt-1">*Harga sudah termasuk semua fasilitas</p>
              </div>
              ...
              <div className="flex gap-3 w-full md:w-auto">
                ...
                <a 
                  href={`https://wa.me/6281234567890?text=Halo%20saya%20tertarik%20dengan%20paket%20${paket.nama}%20ke%20${paket.tujuan}`}
                  target="_blank"
          ...
          <div className="mt-4 text-xs text-gray-400 flex items-center gap-1">
            ...
            Harga dapat berubah sesuai musim liburan. Minimal 20 orang.
          </div>
        </div>
      </div>
    </div>
...
export default ModalDetailPaket;